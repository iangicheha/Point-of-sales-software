...
import {
  Box, Typography, Button, Paper, Stack, Grid, TextField, Autocomplete, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, IconButton, Snackbar, Alert
} from '@mui/material';
...
export const NewPurchaseOrder = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        // In a real app, you might want dedicated supplier/item endpoints
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setTimeout(() => navigate('/purchase-order'), 1500);
    } catch (err) {
    ...
    <Box sx={{ p: { xs: 1, md: 3 } }}>
      <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ mb: 3 }}>
        ...
        <Stack direction="row" spacing={2}>
          <Button variant="outlined" onClick={() => navigate('/purchase-order')}>Cancel</Button>
          <Button variant="contained" startIcon={<Save />} onClick={handleSave} disabled={!selectedSupplier || orderItems.length === 0 || loading}>
